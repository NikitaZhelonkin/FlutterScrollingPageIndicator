...
class _ScrollingPageIndicatorState extends State<ScrollingPageIndicator> {
  ...
  Widget build(BuildContext context) {
    final int? itemCount = widget.itemCount! >= widget.visibleDotCount
        ? widget.visibleDotCount
        : widget.itemCount;
    final double width =
        (itemCount! - 1) * widget.dotSpacing + widget.dotSelectedSize;
...
class _Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double height =
        orientation == Axis.vertical ? size.width : size.height;
    ...
    final double scaleDistance =
        (_widget.dotSpacing + (_widget.dotSelectedSize - _widget.dotSize) / 2) *
            0.7;
    final double smallScaleDistance = _widget.dotSelectedSize / 2;

    final int firstVisibleDotPos =
    ...
    for (int i = firstVisibleDotPos; i <= lastVisibleDotPos; i++) {
      final double dot = getDotOffsetAt(i);
      if (dot >= _visibleFramePosition! &&
        ...
        if (_widget.itemCount! > _widget.visibleDotCount) {
          ...
          if (dot - _visibleFramePosition! < currentScaleDistance) {
            final double calculatedDiameter = diameter *
  ...
  double getDotScaleAt(int index) {
    final int position = page.floor();
    final double offset = page - position;
    if (index == position) {
      return 1 - offset.abs();
  ...
  void adjustFramePosition(double page, double width) {
    ...
    if (_widget.itemCount! <= _widget.visibleDotCount) {
      ...
      final double center =
          getDotOffsetAt(position) + _widget.dotSpacing * offset;
      _visibleFramePosition = center - width / 2;

      // Block frame offset near start and end
      final int firstCenteredDotIndex = (_widget.visibleDotCount / 2).floor();
      final double lastCenteredDot =
